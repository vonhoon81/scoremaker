...
exact content width, and the scale follows from LINES_PER_PAGE. Breaks are forced at
repeat signs and kept out of any mark drawn above the staff, so a chord symbol or a
section label is never sliced by a system break.
"""
...
ROUTE = None  # this arrangement has no segno, coda or D.S. to spell out
...
ROW0, ROW1 = 6, 334  # the panorama rows that carry ink
LINES_PER_PAGE = 7
MAX_STRETCH = 1.18
...
def build_lines():
    ...
    for i in repeat_bars(pano, bars):
        if i >= FORCED_GAP and i - last >= FORCED_GAP:
            forced.append(i)
            last = i
    spans = stitch.label_spans(pano, band=(ROW0, 135))
...
def main():
    ...
    for p, items in enumerate(pages):
        ...
        if p == 0:
            ...
            d.text((MARGIN_X, MARGIN_TOP + 92), CREDIT, font=foot_f,
                   fill=(120, 120, 120))
