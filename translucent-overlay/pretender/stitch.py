...
This arrangement is a practice chart -- repeat counts ("12x", "8x", "9x"), a five-bar
multi-rest, first/second-time voltas, a segno and a D.S. al Coda -- and the video plays
all of it. Reading the bar numbers off the pages, it goes:

    pages  0- 5   bars  1-27     first time through
    pages  6- 8   bars 19-31     back for the second volta, ending on D.S. al Coda
    pages  9-11   bars  9-20     the D.S., from the segno
    pages 12-18   bars 30-56     the coda
    pages 19-21   bars 46-56     and once more
    pages 22-26   bars 57-59     the ending, held (these five are the same page: the ink
                                 profile jumped because the player moved, not the score)

So pages neither march through the score nor split into two clean passes, and single
pages cannot be placed reliably either -- the riff repeats enough that one page matches
several positions. What is unambiguous is a *run*: a maximal stretch of pages the video
flipped through without jumping. Those are 4000-9000px of score each, so they place
against the panorama with no contest. Hence:

  1. score every consecutive pair by full search over their ink masks, using a metric
     that ignores 1px jitter -- each mask is compared against the other's dilation, which
     drops a true flip from ~0.2 to ~0.02 while a jump stays above 0.14. Without the
     dilation the two classes overlap and the D.S. is missed.
  2. cut the pages into runs wherever that score says the pages do not overlap at all.
  3. place run 0 at the origin and register each later run against the panorama so far:
     a coarse pass on column ink profiles, then the 2D metric on the best candidates.
  4. take the per-pixel median of every page covering a column. Each part of the score is
     seen by two to five pages, which is what finally clears the last of the guitar-neck
     residue -- it lands somewhere different on every page.

Barlines are then detected once, on the panorama, where those extra samples have made
them far cleaner than on any single page: where the guitar shows through the translucent
overlay a barline can be too faint to find on the page it came from.
"""
...
MATCH_Y0 = 110  # rows above this hold the residue of the guitar neck; keep them out
...
def panorama():
    ...
    for x0, x1 in zip(edges, edges[1:]):
        ...
        out[:, x0:x1] = np.median(stack, axis=0).astype(np.uint8)
